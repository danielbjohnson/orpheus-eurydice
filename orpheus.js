...
async function fetchData() {
  try {
    const response = await fetch('https://api.example.com/data'); // Pauses until fetch completes
    if (!response.ok) {
...
async function input(buttonA,buttonB){
    try {
        await buttonA.addEventListener("click",buttonClick);
        //await buttonB.addEventListener("click",buttonClick);
    } catch {
        console.log("unfufilled promise")
    }
}
...
function make_choice(){
    ...
    userChoice = input(buttonA,buttonB);
}

function play_scene(scene){
    switch (scene){
        case "SNAKEBITE":
